function [sens_tr, spec_tr, sens_tst, spec_tst] = parallel_classifying_perceptron(eegT, eegNT,beta)
eegTf =  apply_bandpass_filter(eegT);
eegNTf = apply_bandpass_filter(eegNT);
N0 = size(eegNT, 3);
N1 = size(eegT, 3);
Y = [-1*ones(1,N0), ones(1,N1)]';
CV = cvpartition(Y, 'k', 2);
trIdx = CV.training(1);
tstIdx = CV.test(1);
Ytr = Y(trIdx, :);
Ytst= Y(tstIdx, :);
...
for band = 1:size(eegNTf,4)
    ...
    tmp_Ypred_train = arrayfun(@(x) -1*(x==0)+(x==1),tmp_Ypred_train);
    tmp_Ypred0_train = tmp_Ypred_train(Ytr == -1);
    tmp_Ypred1_train = tmp_Ypred_train(Ytr == 1);
    ...
    tmp_Ypred_test = arrayfun(@(x) -1*(x==0)+(x==1),tmp_Ypred_test);
    tmp_Ypred0_test = tmp_Ypred_test(Ytst == -1);
    tmp_Ypred1_test = tmp_Ypred_test(Ytst == 1);
...
[B] = perceptron(Ypred_train,Ytr,0.5);
mu=0;
v = sum(Ypred_train(:,trial).*B,1) - mu;
...
function [B] = perceptron(Ypred_train,Ytr,epsilon)
    B=ones(1,size(Ypred_train,1))';
    mu=0;
    for trial=1:size(Ytr,1)
        v = sum(Ypred_train(:,trial).*B,1) - mu;
        v=-1*(v<0) + 1*(v>0);
        dB = epsilon*(Ytr(trial) - v)*Ypred_train(:,trial);
        B=B+dB;
    end;
end
